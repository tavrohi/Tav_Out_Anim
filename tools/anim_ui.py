...
def windowanimlayout():
    ...
    cmds.button(label = "Anim/Layout Check and Playblast", width=200, bgc=(0.1,0.1,0.1), c=(__name__+".apbui()"))
    ...
    cmds.button(label = "Remove Unload References", width=200, bgc=(0.1,0.1,0.1), c="maya.mel.eval('source \"tools/mod_ui_scripts/xtras.mel\"; remove_unload_references;')")
    cmds.button(label = "Remove Namespace", width=200, bgc=(0.1,0.1,0.1), c="maya.mel.eval('source \"tools/mod_ui_scripts/Finalize.mel\"; sbRemoveNSproc;')")
    cmds.button(label = "Remove Extra Camera", width=200, bgc=(0.1,0.1,0.1), c="maya.mel.eval('source \"tools/mod_ui_scripts/xtras.mel\"; remove_extra_cam;')")
    cmds.button(label = "pk Ghost", width=200, bgc=(0.1,0.1,0.1), c="import tools.pk_ghosts\nreload(tools.pk_ghosts)")
    cmds.button(label = "Fix Mesh", width=200, bgc=(0.1,0.1,0.1), c="maya.mel.eval('source \"tools/anim_ui_scripts/FixMesh.mel\"; deformedmeshremove;')")
...
def windowanimui():
    ...
    print(__name__)
    cmds.workspaceControl('animwin', tabToControl=('AttributeEditor', -1), li=1, r=1, mw=250 , label=WINDOW_SIDE_TITLE, uiScript=(__name__+".windowanimlayout()"))
